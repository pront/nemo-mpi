...
ConnectivityMatrix::ConnectivityMatrix(const nemo::network::Generator& net, const nemo::ConfigurationImpl& conf,
		const Mapper& mapper) :
		m_mapper(mapper), m_maxDelay(0), mhf_weights(WARP_SIZE, 0), md_fcmPlaneSize(0), md_fcmAllocated(0), m_fractionalBits(
				conf.fractionalBits()), m_writeOnlySynapses(conf.writeOnlySynapses()) {
	...
	if ( logging ) {
		...
		std::cout << "Using fixed point format Q" << 31 - m_fractionalBits << "." << m_fractionalBits << " for weights."
				<< std::endl;
	}
	...
	for ( network::synapse_iterator si = net.synapse_begin(); si != net.synapse_end(); ++si ) {
		const Synapse& s = *si;
		setMaxDelay(s);
...
void ConnectivityMatrix::setMaxDelay(const Synapse& s) {
	using boost::format;
	...
	if ( s.delay > MAX_DELAY ) {
		throw nemo::exception(NEMO_INVALID_INPUT,
				str(
						format(
								"Neuron %u has synapses with delay %ums. The CUDA backend supports a maximum of %ums") % s.source % s.delay % MAX_DELAY));
	}
}

size_t ConnectivityMatrix::addForward(const Synapse& s, const DeviceIdx& d_source, const DeviceIdx& d_target,
		size_t& nextFreeWarp, construction::FcmIndex& index, std::vector<synapse_t>& h_targets,
 ...
 * CM is empty.
 */
void ConnectivityMatrix::verifySynapseTerminals(const aux_map& cm, const Mapper& mapper) {
	using boost::format;
	...
	for ( aux_map::const_iterator ni = cm.begin(); ni != cm.end(); ++ni ) {
		...
		if ( !mapper.existingGlobal(source) ) {
			throw nemo::exception(NEMO_INVALID_INPUT, str(format("Invalid synapse source neuron %u") % source));
		}
...
void ConnectivityMatrix::moveFcmToDevice(size_t totalWarps, const std::vector<synapse_t>& h_targets,
	...
	memcpyToDevice(md_fcm.get() + md_fcmPlaneSize * FCM_WEIGHT, reinterpret_cast<const synapse_t*>(&h_weights[0]),
			md_fcmPlaneSize);
}

void ConnectivityMatrix::printMemoryUsage(std::ostream& out) const {
 ...
 * exactly once. */
void ConnectivityMatrix::addAuxillary(const Synapse& s, size_t addr) {
	id32_t id = s.id();
...
ConnectivityMatrix::syncWeights(cycle_t cycle) const {
	if ( cycle != m_lastWeightSync && !mhf_weights.empty() ) {
		...
		memcpyFromDevice(reinterpret_cast<synapse_t*>(&mhf_weights[0]), md_fcm.get() + FCM_WEIGHT * md_fcmPlaneSize,
				md_fcmPlaneSize);
...
float ConnectivityMatrix::getWeight(cycle_t cycle, const synapse_id& id) const {
...
unsigned ConnectivityMatrix::getTarget(const synapse_id& id) const {
	return axonTerminalAux(id).target();
}

unsigned ConnectivityMatrix::getDelay(const synapse_id& id) const {
	return axonTerminalAux(id).delay();
}

unsigned char ConnectivityMatrix::getPlastic(const synapse_id& id) const {
	return axonTerminalAux(id).plastic();
}

void ConnectivityMatrix::clearStdpAccumulator() {
	md_rcm.clearAccumulator();
}

size_t ConnectivityMatrix::d_allocated() const {
	return md_fcmAllocated + md_rcm.d_allocated() + m_gq.allocated() + md_delays->allocated() + m_outgoing.allocated();
}

void ConnectivityMatrix::setParameters(param_t* params) const {
